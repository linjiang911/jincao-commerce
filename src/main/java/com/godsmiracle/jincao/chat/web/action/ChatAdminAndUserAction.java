...
@Controller
@RequestMapping(value="chatAdminAndUserAction",produces={"text/json;charset=UTF-8"})
public class ChatAdminAndUserAction {
	...
	public String login(String account,String pwd,HttpSession session) throws Exception{
		...
		if(adminUserServiec.findOneFrist("account", account)==null||adminUserServiec.findByFristOneHQL("From AdminUser where account=? and password=?", account,pwd)==null){
			AdminUser findByFristOneHQL = adminUserServiec.findByFristOneHQL("From AdminUser where account=? and password=?", account,pwd);
			System.out.println(findByFristOneHQL);
